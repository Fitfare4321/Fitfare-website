...
const perks = [
  ...
  },
  {
    ...
    desc: "Access to partner gyms and health perks.",
  },
  {
    icon: GraduationCap,
    title: "Learning Credits",
    desc: "We invest in your upskilling journey.",
  },
  {
    icon: Globe,
    title: "Diverse Team",
    desc: "Collaborate with talent across the country.",
  },
];
...
const CareerPage = () => {
  ...
  return (
    <section
      className={`relative min-h-screen overflow-hidden transition-colors duration-700 ${isDark ? "bg-[#05070a] text-white" : "bg-slate-50 text-slate-900"}`}
    >
      {/* ── Dynamic Background ── */}
      <div className="absolute inset-0 pointer-events-none">
        <div
          className={`absolute top-0 right-0 w-[800px] h-[800px] rounded-full blur-[160px] opacity-20 ${isDark ? "bg-blue-600/10" : "bg-blue-400/20"}`}
        />
        <div
          className={`absolute bottom-0 left-0 w-[600px] h-[600px] rounded-full blur-[140px] opacity-10 ${isDark ? "bg-purple-600/10" : "bg-purple-400/15"}`}
        />
      ...
      <div className="relative max-w-7xl mx-auto px-6 py-28 lg:py-40">
          ...
          >
            {[
              ...
              <div key={i} className="text-center group">
                <div
                  className={`mb-3 inline-flex w-12 h-12 items-center justify-center rounded-2xl transition-all duration-300 ${isDark ? "bg-slate-900 group-hover:bg-blue-600/20 text-slate-400 group-hover:text-blue-400" : "bg-white group-hover:bg-blue-50 text-slate-400 group-hover:text-blue-600 shadow-sm"}`}
                >
                  <s.icon size={20} />
                </div>
        ...
        <div id="open-positions" className="mb-40">
          <div className="flex flex-col items-center mb-20">
            ...
            {/* Custom Tab Switcher */}
            <div
              className={`p-1.5 rounded-2xl flex gap-2 border ${isDark ? "bg-slate-900/50 border-slate-800" : "bg-white border-slate-200 shadow-lg"}`}
            >
              <button
                      ...
                      <div className="flex flex-col lg:flex-row gap-10 items-start">
                        <div className="flex-1">
                          <div className="flex flex-wrap items-center gap-4 mb-6">
                            <span
                              className={`px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest ${isDark ? "bg-blue-500/10 text-blue-400 border border-blue-500/20" : "bg-blue-50 text-blue-600 border border-blue-100"}`}
                            >
                              {job.tag}
                            </span>
                          ...
                          <p
                            className={`text-base md:text-lg mb-8 leading-relaxed line-clamp-2 ${isDark ? "text-slate-400" : "text-slate-600"}`}
                          >
                            {job.description}
                          </p>
                          ...
                          <div className="flex flex-wrap gap-2">
                            {job.skills.map((s) => (
                              <span
                                key={s}
                                className={`px-3 py-1 rounded-lg text-[11px] font-bold border ${isDark ? "border-slate-800 text-slate-500" : "border-slate-200 text-slate-500"}`}
                              >
                                {s}
                              </span>
                            ))}
        ...
        <div className="mb-40 grid lg:grid-cols-[1fr,1.5fr] gap-20 items-center">
          <div>
            <p className="text-xs uppercase font-black tracking-[0.4em] text-blue-500 mb-6">
              Built for builders
            </p>
            ...
            <p
              className={`text-lg leading-relaxed mb-12 ${isDark ? "text-slate-400" : "text-slate-600"}`}
            >
              We're not just offering a desk; we're offering a launchpad. Join a
              culture that celebrates initiative, ownership, and obsession with
              quality.
            </p>
            ...
            <div className="grid gap-8">
              {perks.map((p, i) => (
                <div key={i} className="flex gap-5 items-start">
                  <div
                    className={`shrink-0 w-12 h-12 rounded-xl flex items-center justify-center ${isDark ? "bg-slate-900 text-blue-400" : "bg-blue-50 text-blue-600"}`}
                  >
                    <p.icon size={22} />
                  </div>
                  <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {benefits.map((b, i) => (
              <div
                key={i}
                className={`group p-10 rounded-[2.5rem] border transition-all ${isDark ? "bg-slate-900/40 border-slate-800 hover:border-slate-700" : "bg-white border-slate-100 shadow-sm hover:shadow-xl"}`}
              >
                <div
                  className={`mb-8 w-14 h-14 rounded-2xl flex items-center justify-center bg-gradient-to-br ${b.gradient} text-white shadow-lg`}
                >
                  <b.icon size={28} />
                </div>
        ...
        ════════════════════════════════════════ */}
        <div
          className={`rounded-[3rem] p-12 md:p-24 overflow-hidden relative ${isDark ? "bg-slate-900/50 border border-slate-800" : "bg-white border border-slate-100 shadow-2xl"}`}
        >
          <div className="relative z-10">
            <div className="text-center max-w-3xl mx-auto mb-20">
              ...
              >
                A transparent, human-centric process designed to respect your
                time and expertise.
              </p>
            </div>
            ...
            <div className="grid md:grid-cols-4 gap-12 relative">
              ...
              {steps.map((s, i) => (
                <div key={i} className="relative z-10 text-center group">
                  <div
                    className={`mx-auto w-24 h-24 rounded-3xl flex items-center justify-center mb-8 border-4 transition-all duration-300 ${isDark ? "bg-slate-900 border-[#05070a] group-hover:border-blue-500/50 group-hover:scale-110" : "bg-white border-slate-100 group-hover:border-blue-100 group-hover:scale-110 shadow-lg"}`}
                  >
                    <s.icon size={32} className="text-blue-500" />
                    <span className="absolute -top-3 -right-3 w-8 h-8 rounded-full bg-blue-600 text-white text-[10px] font-black flex items-center justify-center shadow-lg">
                      {s.num}
                    </span>
                  </div>
                  ...
                  <p
                    className={`text-sm leading-relaxed px-4 ${isDark ? "text-slate-500" : "text-slate-500"}`}
                  >
                    {s.desc}
                  </p>
                </div>
        ...
        <div className="mt-40 grid lg:grid-cols-2 gap-20 items-center">
          <div
            className={`p-10 md:p-16 rounded-[3rem] border relative overflow-hidden ${isDark ? "bg-gradient-to-br from-slate-900 to-slate-800 border-slate-700" : "bg-white border-slate-200 shadow-2xl"}`}
          >
            <div className="relative z-10 space-y-8">
              <div>
                <h3 className="text-3xl font-black tracking-tight text-blue-500 mb-2">
                  Alex Johnson
                </h3>
                <p className="text-xs font-bold tracking-widest uppercase opacity-40">
                  Software Developer • alex@email.com
                </p>
              </div>
                ...
                <div className="flex flex-wrap gap-2 pt-4">
                  {["React", "TypeScript", "Tailwind", "Node.js"].map(
                    (skill) => (
                      <span
                        key={skill}
                        className="px-3 py-1 rounded-lg bg-blue-500/10 text-blue-400 text-[10px] font-black border border-blue-500/20"
                      >
                        {skill}
                      </span>
                    ),
              ...
              <div className="pt-10 flex flex-col items-center gap-6">
                ...
                <div className="flex gap-3 items-start p-6 rounded-2xl bg-blue-500/5 border border-blue-500/10">
                  ...
                  <p className="text-xs leading-relaxed opacity-60">
                    <span className="font-black text-blue-500">Pro Tip:</span>{" "}
                    Hyperlink your GitHub and Portfolio URLs so we can see your
                    amazing work instantly.
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="space-y-10">
            ...
            >
              We review every application manually. A clean, structured resume
              helps us understand your impact faster.
            </p>
            ...
            <div className="grid gap-4">
              {[
                {
                  t: "Measurable Impact",
                  d: "Use numbers (e.g., 'Optimized performance by 40%').",
                },
                {
                  t: "Clean Layout",
                  d: "One page is usually plenty. Keep it scannable.",
                },
                {
                  t: "Relevant Skills",
                  d: "Highlight the tech we actually use.",
                },
                {
                  t: "Your Passion",
                  d: "Show us why you care about the fitness industry.",
                },
              ].map((item, i) => (
                <div
                  key={i}
                  className={`p-6 rounded-2xl border transition-all ${isDark ? "bg-slate-900/30 border-slate-800 hover:border-slate-700" : "bg-white border-slate-100 hover:border-slate-200 shadow-sm"}`}
                >
