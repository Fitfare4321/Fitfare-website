    ...
    <motion.div
      ...
      onDragEnd={(_, info) => {
        ...
        if (info.offset.x < -SWIPE_THRESHOLD) {
          setActive(Math.min(active + 1, cities.length - 1));
        } else if (info.offset.x > SWIPE_THRESHOLD) {
          setActive(Math.max(active - 1, 0));
        }
      }}
      className="absolute cursor-pointer will-change-transform"
      style={{ transformStyle: "preserve-3d", touchAction: isMobile ? "none" : "auto" }}
      initial={{
        x: 0,
        scale: 0.7,
        opacity: 0,
    ...
    >
      ...
      >
        ...
        <div className="p-8 flex flex-col h-full">
          ...
          <h3
            className={`text-2xl font-bold mb-2 ${isMoon ? "text-white" : "text-slate-900"
              }`}
          >
            {city.name}
          </h3>
          ...
          <p
            className={`text-sm font-medium mb-4 ${isMoon ? "text-gray-400" : "text-slate-500"
              }`}
          >
            {city.label}
          </p>
          ...
          <p
            className={`text-sm leading-relaxed ${isMoon ? "text-gray-300" : "text-slate-600"
              }`}
          >
...
const CitiesSection = () => {
  ...
  return (
    <section
      ref={ref}
      className={`relative z-0 py-16 md:py-28 overflow-hidden transition-all duration-500
        ${isMoon
    ...
    >
      {/* Heading */}
    <div className="max-w-7xl mx-auto px-6 mb-10 md:mb-20 flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
        ...
        <p
          className={`max-w-md text-lg ${isMoon ? "text-gray-300" : "text-gray-600"
            }`}
        >
